...
def MOR(load_file_name):
    ...
    for i in range(T1_num):
        w1.cell(45 + (3 * i), 1).value = str(i+1) + "&" + str(i+9) + "T2"
...
def main():
    ...
    print("mode를 입력 하세요 : MOR : 1, PRD : 2")
    ipnut_mode = int(input())

    file_list = os.listdir()
    for idx, data in enumerate(file_list):
        print(data, " : 선택하려면 다음 번호 입력 ", idx)
    print("파일 번호를 입려하세요 : ")
    idx_file = int(input())
    if ipnut_mode == 1:
        MOR(file_list[idx_file])
    else:
        PRD(file_list[idx_file])
