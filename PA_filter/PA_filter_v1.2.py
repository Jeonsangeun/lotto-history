...
class MyApp(QWidget):
    ...
    def initUI(self):
        ...
        UPS_clear = QPushButton("Clear UPS")
        UPS_clear.clicked.connect(self.Clear_UPS)

        master_url = QPushButton("Master list URL")
        master_url.clicked.connect(self.url_for_mas)

        Divide_ALL = QPushButton("ALL UPS")
        Divide_ALL.clicked.connect(self.Divided_All)
        ...
        hbox3.addStretch(1)
        hbox3.addWidget(UPS_clear)

        hbox4 = QHBoxLayout()
        hbox4.addWidget(Divide_ALL)
        hbox4.addStretch(1)
    ...
    def Clear_UPS(self):
        self.UPS_rack = {'UPS1': [], 'UPS2': [], 'UPS3': [], 'UPS4': []}
        self.COLO_log = [0] * len(Available_CELL)
        self.Colo_select.setText('Select Colo : ')
        self.UPS_apply.setText('현재 적용된 COLO : ')
    ...
    def Divided_EG(self, ups):
        ...
        if self.res_mas_url.text() == '':
            ...
            index_BT = list(load_ex.keys()).index('BGTeam')
            ...
            EG_set.remove('')
            EG_set.remove('Fabric Network Devices')

            for loc in self.UPS_rack[ups]:
                Location.append(loc[0] + '.' + loc[1])

            for value in EG_set:
                flag = 0 # Valid EG filter
                wb = excel.Workbook()
                    ...
                    if not isNaN(data_res[index_DcL]):
                        if data_res[index_DcL][6:15] in Location:
                            if data_res[index_SN] == value:
                                flag = 1
                                if data_res[index_BT][0] == '=':
                                    data_res[index_BT] = ';' + data_res[index_BT]
                                ws.append(list(data_res))
                if flag == 1:
                    wb.save("PUS04-XXXX-DD-MM-YYYY IT Power Maintenance-" + value + ".xlsx")
            QMessageBox.about(self, 'Success', 'Complete to save')

    def Divided_All(self):
        if self.res_mas_url.text() == '':
            QMessageBox.about(self, 'Alert', 'Please select URL')
        else:
            load_ex = pd.read_excel(self.res_mas_url.text())
