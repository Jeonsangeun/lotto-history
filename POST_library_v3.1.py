...
class MyApp(QWidget):
    ...
    def __init__(self):
        ...
        self.meta_data = []
        self.folder = QLineEdit('input', self)
        ...
        self.ex_row = 0
        self.manu, self.relate_task, self.note_his, self.date, self.DC_code = 2, 3, 4, 6, 7
    ...
    def setting_address(self):
        self.address = self.folder.text() + '\POST_CODE.xlsx'
    ...
    def input_data(self):
        ...
        load_ws.cell(self.ex_row, self.manu + 1, value=self.name.text())
        load_ws.cell(self.ex_row, self.note_his + 1, value=self.note.text())
        load_ws.cell(self.ex_row, self.date + 1, value=self.today)
        load_ws.cell(self.ex_row, self.DC_code + 1, value='PUS04')

        self.work_book.save(self.address)
        self.meta_data = pd.read_excel(self.address, sheet_name=None, na_values="TT")

    def search(self):
        if self.name.text() == 'None' or self.meta_data == []:
        ...
        if self.name.text() == 'None' or self.meta_data == []:
            ...
            for idx, value in enumerate(data.values):
                if value[0] == find_code: # BIOS code self.name
                    self.ex_row = idx + 3
                    if value[self.date] != '':
                        # note += ('GDCO : ' + value[self.relate_task][-10:] + " | ")
                        note += ('date : ' + str(value[self.date])[:10] + " | ")
                        note += ('DC : ' + value[self.DC_code])
                        note += '\n'
                        note += ('note : ' + value[self.note_his])
                        note += '\n'
                    else:
                        pass
            if note == '':
