...
class MyApp(QWidget):
    ...
    def __init__(self):
        ...
        self.folder = QLineEdit('path', self)
        ...
        if os.path.isfile(now_path + '\\address_file.txt'):
            with open(now_path + '\\address_file.txt', 'r') as file:
                self.folder.setText(file.readline()[:-1])
        ...
        self.note = QTextEdit('note', self)
        self.URL = QLineEdit('GDCO#', self)
    ...
    def initUI(self):
        ...
        hbox7 = QHBoxLayout()
        hbox7.addWidget(QLabel('GDCO URL'))
        hbox7.addWidget(self.URL)
        hbox7.addWidget(S_Button)
        hbox7.addStretch(1)
        ...
        self.setGeometry(100, 100, 275, 600)
        self.show()
    ...
    def input_data(self):
        ...
        load_ws.cell(self.ex_row, self.relate_task, value=self.URL.text())
        load_ws.cell(self.ex_row, self.note_his, value= "'" + self.note.toPlainText())
        ...
        if self.m_name.text() == 'None' or self.meta_data == []:
            ...
            for value in data.values:
                if value[0] == find_code:
                    ...
                    description += '\n'
            if description == '':
                self.S_des.setPlainText('No found')
            else:
                self.S_des.setPlainText(description)
        ...
        if self.m_name.text() == 'None' or self.meta_data == []:
            ...
            for idx, value in enumerate(data.values):
                if value[0] == find_code: # BIOS code self.name
                    self.ex_row = idx + 3
                    if value[self.manu - 1] == self.m_name.text():
                        note += ('date : ' + str(value[self.date - 1])[:10] + "\n")
                        note += ('DC : ' + value[self.DC_code - 1] + '\n')
                        note += ('note : ' + value[self.note_his - 1] + '\n')
                        note += ('GDCO : ' + value[self.relate_task-1] + "\n")
                        note += ("------------------------------------\n")
                    else:
                        pass
            if note == '':
                self.D_des.setPlainText('No found')
            else:
                self.D_des.setPlainText(note)
